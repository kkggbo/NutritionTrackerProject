...
> 请求方式：**POST**
>
> 接口描述：设置或更新用户的基本信息（性别、年龄、身高、体重、目标），以便进行个性化营养规划与健康管理。
...
| 参数名    | 说明              | 类型              | 是否必须 | 备注                       |
| ------ | --------------- | --------------- | ---- | ------------------------ |
| userId | 用户 ID           | number (long)   | 是    | 通常由前端登录状态提供              |
| gender | 性别（1=男，2=女）     | number (int)    | 是    | 前端下拉选择“男/女”，提交前转为数字      |
| age    | 年龄              | number (int)    | 是    | 范围：10–90，前端校验            |
...
| activityLevel   | 活动水平 | number (int)    | 是    | 前端下拉选择 |
